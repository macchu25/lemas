...
export default function InteractiveHeroCode() {
  ...
  return (
    <div className="w-full overflow-hidden rounded-2xl border border-[var(--color-border)] bg-[#070913] shadow-2xl shadow-cyan-950/20">
      {/* Code Header Bar */}
      <div className="flex items-center justify-between border-b border-[var(--color-border)] bg-[#0d1020] px-4 py-2.5">
        {/* Window controls */}
        <div className="flex items-center gap-3">
          ...
          <span className="font-mono text-xs text-slate-400">{current.file}</span>
        </div>

        {/* Tab switchers */}
        <div className="flex items-center gap-1 bg-[#05060d] p-1 rounded-lg border border-[var(--color-border)]">
        ...
        <div className="flex items-center gap-1.5">
          <button
            onClick={handleRun}
            disabled={isRunning}
            className="flex items-center gap-1 px-2.5 py-1 text-xs font-mono rounded bg-cyan-500/15 text-cyan-400 hover:bg-cyan-500/25 border border-cyan-500/30 transition-all"
          ...
          <button
            onClick={handleCopy}
            className="flex items-center justify-center size-7 rounded text-slate-400 hover:text-white hover:bg-slate-800 transition-all"
            title="Copy snippet"
          >
      ...
      {/* Code Body */}
      <pre className="overflow-x-auto p-5 font-mono text-[13px] leading-[1.7] text-slate-200">
        <code>
          {current.code.split('\n').map((line, i) => (
      ...
      {/* Code Status Footer */}
      <div className="flex items-center justify-between border-t border-[var(--color-border)] bg-[#0d1020] px-4 py-2 text-xs font-mono text-slate-400">
        <div className="flex items-center gap-2">
          ...
          <span className="text-slate-400">Norn.AI Intelligent Gateway</span>
          <span className="text-slate-700">·</span>
          <span className="text-indigo-400 bg-indigo-500/10 px-1.5 py-0.5 rounded text-[11px]">
            failover: active
          </span>
        </div>
