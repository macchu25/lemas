...
export interface ArtQRJob {
  ...
  attempt: number;
  max_attempts: number;
  images?: { url: string; scannable: boolean; decoded?: string }[];
...
export async function createArtQRJob(file: File, style: ArtQRStyle): Promise<ArtQRJob> {
  ...
  const res = await fetch(`${API_BASE}/api/user/art-qr/jobs`, {
    method: 'POST', headers: { Authorization: `Bearer ${token}` }, body: form,
  });
  const data = await res.json();
  if (!res.ok) throw new Error(data.error || 'Không thể tạo Art QR job');
  return data;
}
...
export async function fetchArtQRJob(jobId: string): Promise<ArtQRJob> {
  ...
  const res = await fetch(`${API_BASE}/api/user/art-qr/status?id=${encodeURIComponent(jobId)}`, {
    headers: { Authorization: `Bearer ${token}` }, cache: 'no-store',
  });
  const data = await res.json();
  if (!res.ok) throw new Error(data.error || 'Không thể đọc trạng thái Art QR');
  return data;
}
