...
export default function ImageStudioPage() {
  ...
  const handleGenerate = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const updatedHistory = [result, ...history.filter((h) => h.url !== result.url)].slice(0, 12);
  ...
  return (
    <div className="min-h-full space-y-6 max-w-7xl mx-auto pb-12">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/[0.08] pb-5">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-400">
            Tạo tác phẩm nghệ thuật, concept art và hình ảnh siêu thực 8K với Puter.js AI & FLUX Edge Router
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-7 space-y-5">
          <form onSubmit={handleGenerate} className="p-5 sm:p-6 rounded-3xl border border-white/[0.08] bg-[#0b0d14] space-y-5 shadow-2xl">
            {/* Prompt Input */}
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <label className="text-xs font-bold uppercase tracking-wider text-slate-300 flex items-center gap-2">
                  <Wand2 className="size-3.5 text-cyan-400" />
                  <span>Mô tả ảnh (Prompt)</span>
                </label>
              ...
              <textarea
                ...
                placeholder="Ví dụ: Một chiến binh samurai cyberpunk đứng trên đỉnh tòa nhà Tokyo tương lai trong đêm mưa neon rực rỡ, góc máy cinematic 8k, phong cách anime makoto shinkai..."
              ...
              <div className="flex items-center gap-1.5 flex-wrap pt-1 text-[11px]">
                <span className="text-slate-500 flex items-center gap-1">
                  <Flame className="size-3 text-amber-400" /> Gợi ý nhanh:
                </span>
                <button
                  type="button"
            ...
            <div className="space-y-2.5">
              <label className="text-xs font-bold uppercase tracking-wider text-slate-300 flex items-center gap-2">
                ...
                <span>Tỉ Lệ Khung Hình (Aspect Ratio)</span>
              </label>
            ...
            <div className="space-y-3 pt-1">
              <div className="flex items-center justify-between">
                ...
                >
                  ...
                  <span>{showAdvanced ? 'Ẩn nâng cao' : 'Nâng cao'}</span>
              ...
              {showAdvanced && (
                <div className="p-3.5 rounded-2xl border border-white/[0.08] bg-black/40 space-y-2 animate-fade-in">
                  <label className="text-xs font-semibold text-slate-300">
                    Negative Prompt (Những chi tiết không muốn xuất hiện)
                  </label>
                  <input
                    type="text"
                    value={negativePrompt}
                    onChange={(e) => setNegativePrompt(e.target.value)}
                    placeholder="blurry, distorted face, extra fingers, watermark, text, low quality"
            ...
            >
              {loading ? (
                <>
                  ...
                  <span>Đang khởi tạo tác phẩm với Puter.js AI...</span>
                </>
              ) : (
                <>
                  <Sparkles className="size-4.5" />
                  <span>Tạo Ảnh Ngay (Generate Image)</span>
                </>
              )}
            </button>
          </form>
        ...
        <div className="lg:col-span-5 space-y-5">
          <div className="p-5 sm:p-6 rounded-3xl border border-white/[0.08] bg-[#0b0d14] space-y-4 shadow-2xl">
            <div className="flex items-center justify-between">
              <h2 className="text-xs font-bold uppercase tracking-wider text-slate-300 flex items-center gap-2">
                <ImageIcon className="size-3.5 text-cyan-400" />
                <span>Kết Quả Tác Phẩm</span>
              </h2>
              ...
              {currentImage && (
                <div className="flex items-center gap-1.5">
                  ...
                  <button
                    ...
                    title="Tải ảnh độ phân giải cao"
                    className="p-1.5 rounded-lg border border-white/10 bg-white/5 text-slate-400 hover:text-white hover:border-cyan-500/40 transition-colors cursor-pointer"
                  >
            ...
            <div className="relative w-full rounded-2xl overflow-hidden border border-white/10 bg-black/60 aspect-[16/10] flex items-center justify-center group">
              {loading ? (
                ...
                <div className="flex flex-col items-center gap-2 p-6 text-center text-slate-500">
                  ...
                  <p className="text-xs">Chưa có ảnh nào được tạo</p>
                  <p className="text-[11px] text-slate-600">Nhập mô tả ở khung bên trái và bấm Tạo Ảnh Ngay</p>
                </div>
              )}
            </div>
          ...
          {/* History Gallery */}
          {history.length > 0 && (
            <div className="p-5 rounded-3xl border border-white/[0.08] bg-[#0b0d14] space-y-3 shadow-xl">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold uppercase tracking-wider text-slate-300">
                  Lịch Sử Trong Phiên ({history.length})
                </span>
                <button
                  onClick={() => {
                    setHistory([]);
