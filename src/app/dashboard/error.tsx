...
export default function DashboardError({
  ...
  return (
    <div className="h-full w-full min-h-[400px] flex flex-col items-center justify-center p-6 text-center bg-[#0a0c12] rounded-2xl border border-rose-500/20 shadow-xl">
      ...
      <h2 className="text-base font-bold text-white mb-2">Đã xảy ra lỗi tại trang này</h2>
      <p className="text-xs text-slate-400 max-w-md mb-5 leading-relaxed">
        {error?.message || 'Không thể tải thành phần bảng điều khiển.'}
      </p>
      <div className="flex items-center gap-3">
        <button
          type="button"
          onClick={() => reset()}
          className="px-4 py-2.5 rounded-xl text-xs font-bold bg-cyan-500 hover:bg-cyan-400 text-slate-950 flex items-center gap-2 transition-all shadow-lg shadow-cyan-500/20 active:scale-95"
        >
